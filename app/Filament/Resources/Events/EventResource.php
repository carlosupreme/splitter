...
use App\Filament\Resources\Events\Pages\EventBudget;
...
class EventResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->modifyQueryUsing(fn (Builder $query) => $query->where(function ($q) {
                $q->where('organizer_id', auth()->id()) // Events I organize
                    ->orWhereHas('acceptedInvitees', function ($inviteeQuery) {
                        $inviteeQuery->where('user_id', auth()->id());
            ...
            ->columns([
                ...
                TextColumn::make('role')
                    ->label('Your Role')
                    ->badge()
                    ->getStateUsing(fn (Event $record): string => $record->organizer_id === auth()->id() ? 'Organizer' : 'Attendee'
                    )
                    ->colors([
                        'primary' => 'Organizer',
                        'success' => 'Attendee',
            ...
            ->recordActions([
                ViewAction::make()
                    ->slideOver()
                    ->modalWidth('7xl')
                    ->visible(fn (Event $record): bool => $record->canUserAccessBudget(auth()->id())),

                EditAction::make()
                    ->slideOver()
                    ->modalWidth('7xl')
                    ->visible(fn (Event $record): bool => $record->organizer_id === auth()->id()),

                Action::make('budget')
                    ->url(fn (Event $record): string => EventBudget::getUrl(['record' => $record]))
                    ->label('Budget')
    ...
    public static function getPages(): array
    {
        return [
            ...
            'budget' => EventBudget::route('/{record}/budget'),
