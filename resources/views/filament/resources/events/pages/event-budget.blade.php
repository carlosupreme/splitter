<x-filament-panels::page>
    <div class="space-y-4 lg:space-y-6">
        {{-- Budget Summary Card --}}
        <div
            class="bg-white dark:bg-gray-900 overflow-hidden shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="p-4 lg:p-6">
                <h3 class="text-lg lg:text-xl font-semibold text-gray-900 dark:text-white mb-4">💰 Tu Resumen de
                    Presupuesto</h3>

                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
                    <div class="bg-blue-50 dark:bg-blue-900/20 p-3 lg:p-4 rounded-lg">
                        <div class="text-xs lg:text-sm font-medium text-blue-600 dark:text-blue-400">Total que Debes
                        </div>
                        <div class="text-lg lg:text-2xl font-bold text-blue-900 dark:text-blue-100">
                            ${{ number_format($budgetSummary['total_owed'], 2) }}</div>
                    </div>

                    <div class="bg-green-50 dark:bg-green-900/20 p-3 lg:p-4 rounded-lg">
                        <div class="text-xs lg:text-sm font-medium text-green-600 dark:text-green-400">Total que
                            Pagaste
                        </div>
                        <div class="text-lg lg:text-2xl font-bold text-green-900 dark:text-green-100">
                            ${{ number_format($budgetSummary['total_paid'], 2) }}</div>
                    </div>
                    ...
                    <div class="bg-red-50 dark:bg-red-900/20 p-3 lg:p-4 rounded-lg">
                        ...
                        <div class="text-lg lg:text-2xl font-bold text-red-900 dark:text-red-100">
                            ${{ number_format($budgetSummary['remaining_to_pay'], 2) }}</div>
                    </div>

                    <div class="bg-yellow-50 dark:bg-yellow-900/20 p-3 lg:p-4 rounded-lg">
                        <div class="text-xs lg:text-sm font-medium text-yellow-600 dark:text-yellow-400">Crédito a
                            Recibir
                        </div>
                        <div class="text-lg lg:text-2xl font-bold text-yellow-900 dark:text-yellow-100">
                            ${{ number_format($budgetSummary['credits_to_receive'], 2) }}</div>
                    </div>
                </div>
                ...
                <div class="mt-4 flex justify-center">
                    ...
                    @elseif($budgetSummary['status'] === 'partial')
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100">
        ...
        <div
            class="bg-white dark:bg-gray-900 overflow-hidden shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="p-4 lg:p-6">
                <h3 class="text-lg lg:text-xl font-semibold text-gray-900 dark:text-white mb-4">📊 Totales del
                    Evento</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="text-center bg-red-50 dark:bg-red-900/10 p-4 rounded-lg">
                        ...
                        <div class="text-xl lg:text-2xl font-bold text-red-600 dark:text-red-400">
                            ${{ number_format($event->getTotalExpenses(), 2) }}</div>
                    </div>
                    ...
                    <div class="text-center bg-green-50 dark:bg-green-900/10 p-4 rounded-lg">
                        ...
                        <div class="text-xl lg:text-2xl font-bold text-green-600 dark:text-green-400">
                            ${{ number_format($event->getTotalIncomes(), 2) }}</div>
                    </div>
                    ...
                    <div class="text-center bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg">
                        ...
                        <div
                            class="text-xl lg:text-2xl font-bold {{ $netBalance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                    ...
                    @foreach($attendeeStatus as $person)
                        <div class="relative flex flex-col items-center group">
                            ...
                            <div class="mt-2 text-center max-w-20 lg:max-w-24">
                                <div
                                    class="font-medium text-xs lg:text-sm text-gray-900 dark:text-white {{ $person['user']->id === auth()->id() ? 'underline decoration-blue-500' : '' }} truncate">
                                ...
                                @if($person['amount'] > 0)
                                    <div class="text-xs font-medium mt-1 {{
                                        ...
                                        {{ $person['status'] === 'creditor' ? 'Le deben' : 'Debe' }}
                                        ${{ number_format($person['amount'], 2) }}
                                    </div>
                                @else
                            ...
                            {{-- Hover tooltip --}}
                            <div
                                class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                                <div class="bg-black text-white text-xs rounded-lg px-3 py-2 whitespace-nowrap">
                                    ...
                                    <div class="{{ $person['net_amount'] >= 0 ? 'text-green-400' : 'text-red-400' }}">
                                        Neto: {{ $person['net_amount'] >= 0 ? '+' : '' }}
                                        ${{ number_format($person['net_amount'], 2) }}
                                    </div>
                                    {{-- Arrow --}}
                                    <div
                                        class="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-black"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
                ...
                <div class="mt-6 flex justify-center">
                    <div class="flex flex-col lg:flex-row items-center gap-3 lg:gap-6 text-xs lg:text-sm">
                        <div class="flex items-center gap-2">
                            <div
                                class="w-3 h-3 lg:w-4 lg:h-4 rounded-full bg-green-500 border-2 border-green-400"></div>
        ...
        <div
                ...
                <div class="flex flex-col lg:flex-row lg:items-center gap-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 flex-1">
                        ...
                        <div>
                            <select id="filter-attendee"
                                    class="w-full text-sm px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                    onchange="applyFilters()">
                                <option value="">👥 Todos los Asistentes</option>
                                @foreach($allAttendees as $attendee)
                                    <option
                                        value="{{ $attendee->id }}">{{ $attendee->name }}{{ $attendee->id === auth()->id() ? ' (Tú)' : '' }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="flex items-center gap-2">
                        <button onclick="clearAllFilters()"
                                class="px-3 py-2 text-sm bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-300 rounded-lg transition-colors">
                            🗑️ Limpiar
                        </button>
            ...
            <div class="lg:hidden border-b border-gray-200 dark:border-gray-700 p-4">
                <select id="mobile-tab-selector"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-red-500"
                        onchange="showTabFromSelect(this.value)">
                    <option value="pending">⏳ Pagos Pendientes (@if($pendingExpenses->count() > 0)
                ...
                <div class="overflow-x-auto">
                    <nav class="flex space-x-4 lg:space-x-8 px-4 lg:px-6 min-w-max" aria-label="Tabs">
                        <button onclick="showTab('pending')" id="pending-tab"
                                class="tab-button flex-shrink-0 py-4 px-3 lg:px-1 border-b-2 font-medium text-sm whitespace-nowrap border-red-500 text-red-600 dark:text-red-400">
                            ⏳ Pagos Pendientes
                            @if($pendingExpenses->count() > 0)
                                <span
                                    class="ml-2 px-2 py-1 bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100 rounded-full text-xs font-medium">
                                    {{ $pendingExpenses->count() }}
                                </span>
                            @endif
                        </button>
                        <button onclick="showTab('expenses')" id="expenses-tab"
                                class="tab-button flex-shrink-0 py-4 px-3 lg:px-1 border-b-2 font-medium text-sm whitespace-nowrap border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300">
                            🧾 Todos los Gastos
                            <span
                                class="ml-2 px-2 py-1 bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-100 rounded-full text-xs font-medium">
            ...
            <div class="p-4 lg:p-6">
                    ...
                    @if($pendingExpenses->count() > 0)
                        <div class="space-y-4">
                            @foreach($pendingExpenses as $expense)
                                ...
                                <div
                                    class="filterable-expense border-l-4 border-red-400 bg-red-50 dark:bg-red-900/10 border border-gray-200 dark:border-gray-700 rounded-lg p-3 lg:p-4"
                                    ...
                                    <div class="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-3">
                                        <div class="flex-1">
                                            <div class="flex flex-col lg:flex-row lg:items-center gap-2">
                                                ...
                                                <span
                                                    class="text-xs px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full w-fit">
                                            ...
                                            <div class="text-xs lg:text-sm text-gray-500 dark:text-gray-500 mt-1">
                                                Agregado por {{ $expense->creator->name }} • {{ $attendeeCount }}
                                                personas
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex flex-col lg:items-end gap-2">
                                            <div class="text-right">
                                                <div class="text-lg lg:text-xl font-bold text-gray-900 dark:text-white">
                                            ...
                                            <div class="text-right">
                                                ...
                                                <div class="text-xs text-red-600 dark:text-red-400">Falta por pagar
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    @if($userSplit)
                                        <div
                                            class="mt-3 bg-white dark:bg-gray-800 rounded-lg p-3 border border-red-200 dark:border-red-800">
                                            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                                                <div>
                                                    <div class="font-medium text-gray-700 dark:text-gray-300">Tu Parte
                                                    </div>
                                                    <div class="text-lg font-bold">
                                                        ${{ number_format($userSplit->share_amount, 2) }}</div>
                                                </div>

                                                <div>
                                                    <div class="font-medium text-gray-700 dark:text-gray-300">Pagaste
                                                    </div>
                                                    <div class="text-lg font-bold text-green-600">
                                                        ${{ number_format($userSplit->paid_amount, 2) }}</div>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div
                                                        class="text-lg font-bold {{ $userSplit->getRemainingAmount() > 0 ? 'text-red-600' : 'text-yellow-600' }}">
                                                        ${{ number_format($userSplit->getRemainingAmount() > 0 ? $userSplit->getRemainingAmount() : $userSplit->getOverpaidAmount(), 2) }}
                                                    </div>
                                                </div>

                                                <div>
                                                    <div class="font-medium text-gray-700 dark:text-gray-300">Tu
                                                        Estado
                                                    </div>
                                                    <div class="mt-1">
                                    ...
                                    @if($expense->images && $expense->images->count() > 0)
                                        <div class="mt-3 p-3 bg-gray-50 dark:bg-gray-800 rounded border">
                                            <h6 class="font-medium text-gray-700 dark:text-gray-300 mb-2">📎 Fotos del
                                                Gasto ({{ $expense->images->count() }})</h6>
                ...
                <div id="expenses-content" class="tab-content hidden">
                    @forelse($event->expenses as $expense)
                        ...
                        <div
                            class="filterable-expense border border-gray-200 dark:border-gray-700 rounded-lg mb-4 p-3 lg:p-4"
                            data-creator-id="{{ $expense->creator->id }}"
                            data-category="{{ $expense->category }}"
                            data-amount="{{ $expense->amount }}"
                            data-created-date="{{ $expense->created_at->format('Y-m-d') }}">
                            <div class="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-3 gap-3">
                                <div class="flex-1">
                                    ...
                                    <div class="text-xs lg:text-sm text-gray-500 dark:text-gray-500 mt-1">
                                        Agregado por {{ $expense->creator->name }} • Dividido entre {{ $attendeeCount }}
                                        personas
                                    </div>
                                </div>
                                ...
                                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                                    ...
                                    @if(count($creditors) > 0 && count($debtors) > 0)
                                        <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded border">
                                            <h5 class="font-semibold text-blue-900 dark:text-blue-100 mb-2">💡
                                                Sugerencias de Pago</h5>

                                            @foreach($creditors as $creditor)
                                                <div class="text-sm text-blue-800 dark:text-blue-200 mb-1">
                                                    <strong>{{ $creditor['user']->name }}</strong> debe recibir
                                                    ${{ number_format($creditor['amount'], 2) }}
                                                </div>
                                            @endforeach
                                            ...
                                            @foreach($debtors as $debtor)
                                                @if($debtor['user']->id === auth()->id())
                                                    <div class="text-sm text-blue-800 dark:text-blue-200">
                                                        → Deberías pagar ${{ number_format($debtor['amount'], 2) }}
                                                        a {{ $creditors[0]['user']->name ?? 'alguien que pagó demás' }}
                                                    </div>
                                                @endif
                                            @endforeach
                                        </div>
                                    ...
                                    @if($expense->images && $expense->images->count() > 0)
                                        <div class="mt-4 p-3 bg-gray-50 dark:bg-gray-800 rounded border">
                                            <h6 class="font-medium text-gray-700 dark:text-gray-300 mb-2">📎 Fotos
                                                Adjuntas ({{ $expense->images->count() }})</h6>
                                            <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                                @foreach($expense->images as $image)
                                                    <div class="relative group">
                                                        <img src="{{ $image->url }}"
                ...
                <div id="payments-content" class="tab-content hidden">
                    @if($allPayments->count() > 0)
                        <div class="space-y-4">
                            @foreach($allPayments as $payment)
                                <div
                                    class="filterable-payment flex flex-col lg:flex-row lg:items-start lg:justify-between p-3 lg:p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 shadow-sm hover:shadow-md transition-shadow"
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="mt-2 flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400">
                                            <span
                                                class="flex items-center gap-1">📅 {{ $payment->paid_at->format('j M Y, g:i A') }}</span>
                                            <span
                                                class="flex items-center gap-1">🏷️ {{ ucfirst($payment->expense->category) }}</span>
                                            <span
                                                class="flex items-center gap-1">📝 Agregado por {{ $payment->expense->creator->name }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-3 lg:mt-0 lg:ml-4 flex flex-col items-start lg:items-end gap-2">
                                        ...
                                        @else
                                            <span
                                                class="px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-100">
                                                Pago de Amigo
                                            </span>
                                        @endif
                    ...
                    @else
                        <div class="text-center py-10 text-gray-500 dark:text-gray-400">
                            <p>Aún no se han registrado pagos. ¡Usa <span class="font-semibold">"Registrar Pago"</span>
                                para registrar tus contribuciones!</p>
                        </div>
                    @endif
                </div>
            </div>
